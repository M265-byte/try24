...
# Composite pearls onto a background and return PIL image
def composite_pearl_bg(bg_path, pearl_paths, positions, pearl_size=(110,110)):
    bg = load_img(bg_path)
    if bg is None:
        return None
    out = bg.copy()
    for ppath, pos in zip(pearl_paths, positions):
        p = load_img(ppath)
        if p is not None:
            try:
                p_resized = p.resize(pearl_size).convert("RGBA")
                out.paste(p_resized, pos, p_resized)
            except Exception:
                out.paste(p.resize(pearl_size), pos)
    return out

# -----------------------
# Session defaults
# -----------------------
if "scene" not in st.session_state: st.session_state.scene = "menu"
if "character" not in st.session_state: st.session_state.character = None
if "kids_q" not in st.session_state: st.session_state.kids_q = 0
if "ship_q" not in st.session_state: st.session_state.ship_q = 0
if "pearls_answered" not in st.session_state: st.session_state.pearls_answered = 0
if "pearl_start" not in st.session_state: st.session_state.pearl_start = None
if "hearts" not in st.session_state: st.session_state.hearts = 4
if "current_pearl" not in st.session_state: st.session_state.current_pearl = None
if "congrats_time" not in st.session_state: st.session_state.congrats_time = None

# -----------------------
# Static data (questions + pearl positions)
# -----------------------
KIDS_QUESTIONS = [
...
# SIGNIN
    ...
    with main:
        ...
        st.session_state._email = st.text_input("Enter Email", key="email")
        st.session_state._pwd = st.text_input("Enter Password", type="password", key="pwd")
    with side:
        if st.button("Submit"):
...
if st.session_state.scene == "pearlgame":
    bg_img = load_img(f"pearlgame{suf}1.png")
    pearl_imgs = [load_img(f"pearl{i+1}.png") for i in range(4)]

    main, side = layout([4,1])
    with main:
        # Show background
        if bg_img:
    ...
    with side:
        ...
        lost = elapsed // 30
        st.session_state.hearts = max(4 - lost, 0)
        st.markdown(f"<div style='text-align:center; color:red; font-size:18px;'>{'❤️ '*st.session_state.hearts}</div>", unsafe_allow_html=True)
        st.markdown("---", unsafe_allow_html=True)

        # Pearl buttons
        for idx, p_img in enumerate(pearl_imgs):
            disabled = idx < st.session_state.pearls_answered
            if st.button(f"Pearl {idx+1}", key=f"pearl_btn_{idx}", disabled=disabled):
                st.session_state.current_pearl = idx

        # Show question per pearl
        ...
        if st.session_state.hearts <= 0:
            st.error("All hearts lost. Proceeding to ship.")
            if st.button("Proceed to Ship (continue)"):
                next_scene("ship")
...
if st.session_state.scene == "ship":
    main, side = layout()
    with main:
        show(load_img(f"ship{suf}1.png"))
    with side:
...
# CONGRATS1 -> wait -> CONGRATS2
if st.session_state.scene == "congrats1":
    main, side = layout()
    with main:
        show(load_img(f"congrats{suf}1.png"))
